...
def process_daily_dialogue(path):
    with open(path, "r") as file:
        data = file.readlines()
    corpus = []
    for d in tqdm(data):
        res = d.strip().split("__eou__")
        res = [s.strip() for s in res if s != ""]
        corpus.append(res)
    return corpus


def process_alpaca(path):
    with open(path, "r") as file:
        data = file.read()
    ...
    corpus = []
    for d in data:
        inp = d["instruction"] + d["input"]
        outp = d["output"]
        corpus.append([inp, outp])
    return corpus
...
if __name__ == "__main__":
    ...
    save_pkl(path, corpus)
    zh_wiki = load_pkl("/data/home/ze.song/data/corpus/zh_wiki.pkl")
    zh_wiki = [s[:2] for s in zh_wiki if len(s) >= 2]
    path = "/data/home/ze.song/data/corpus/dialogue/wiki_dia.pkl"
    save_pkl(path, zh_wiki)

    # belle
    data = datasets.load_dataset("BelleGroup/train_0.5M_CN")
    corpus = list(
        zip(
    ...
    corpus = [list(i) for i in corpus]
    corpus = [[i[0] + i[1], i[2]] for i in corpus]
    path = "/data/home/ze.song/data/corpus/dialogue/train_0.5M_CN.pkl"
    save_pkl(path, corpus)

    data = datasets.load_dataset("BelleGroup/train_1M_CN")
    corpus = list(
        zip(
            data["train"]["instruction"],
            data["train"]["input"],
    ...
    corpus = [list(i) for i in corpus]
    corpus = [[i[0] + i[1], i[2]] for i in corpus]
    path = "/data/home/ze.song/data/corpus/dialogue/train_1M_CN.pkl"
    save_pkl(path, corpus)

    data = datasets.load_dataset("BelleGroup/school_math_0.25M")
    corpus = list(
        zip(
            data["train"]["instruction"],
            data["train"]["input"],
    ...
    corpus = [list(i) for i in corpus]
    corpus = [[i[0] + i[1], i[2]] for i in corpus]
    path = "/data/home/ze.song/data/corpus/dialogue/school_math_0.25M.pkl"
    save_pkl(path, corpus)

    data = datasets.load_dataset("BelleGroup/multiturn_chat_0.8M")
    corpus = list(
        zip(
            data["train"]["instruction"],
            data["train"]["input"],
    ...
    corpus = [list(i) for i in corpus]
    corpus = [[i[0] + i[1], i[2]] for i in corpus]
    path = "/data/home/ze.song/data/corpus/dialogue/multiturn_chat_0.8M.pkl"
    save_pkl(path, corpus)

    data = datasets.load_dataset("BelleGroup/generated_chat_0.4M")
    corpus = list(
        zip(
            data["train"]["instruction"],
            data["train"]["input"],
    ...
    corpus = [list(i) for i in corpus]
    corpus = [[i[0] + i[1], i[2]] for i in corpus]
    path = "/data/home/ze.song/data/corpus/dialogue/generated_chat_0.4M.pkl"
    save_pkl(path, corpus)

    data = datasets.load_dataset("BelleGroup/train_2M_CN")
    corpus = list(
        zip(
            data["train"]["instruction"],
            data["train"]["input"],
    ...
    corpus = [list(i) for i in corpus]
    corpus = [[i[0] + i[1], i[2]] for i in corpus]
    path = "/data/home/ze.song/data/corpus/dialogue/train_2M_CN.pkl"
    save_pkl(path, corpus)

    # alpaca
    path = "/data/home/ze.song/git/alpaca-lora/alpaca_data_cleaned.json"
    corpus = process_alpaca(path)
    path = "/data/home/ze.song/data/corpus/dialogue/alpaca_data.pkl"
    save_pkl(path, corpus)

    path = "/data/home/ze.song/data/raw_corpus/dialogue/trans_chinese_alpaca_data.json"
    corpus = process_alpaca(path)
    path = "/data/home/ze.song/data/corpus/dialogue/cn_alpaca_data.pkl"
    save_pkl(path, corpus)
